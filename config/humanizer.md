# Humanizer — final polish pass (vendored)

Source: https://github.com/blader/humanizer (MIT, v2.9.1), based on Wikipedia's
"Signs of AI writing" guide. Vendored here so the routine gets it without a
plugin install. To update: copy a newer `SKILL.md` in below this preamble and
keep this preamble at the top.

## How this file is used in THIS repo

Run it as the **LAST step on every draft**, after the persona/playbook/guardrails
checks and before the Buffer call. Run it in **EMBEDDED mode**: do the
draft → audit → final loop silently in your head and emit only the finished post.
No draft, no audit bullets, no summary in the posting log. The posting log records
the final text only, exactly as published.

Apply it to the post body, the carousel slide text and caption, and any image
headline text. Do NOT apply it to hashtags, URLs, the first-comment link, slugs,
or JSON.

## PRECEDENCE — read this before applying anything below

The rules below are a *net*, not a *voice*. When they collide with this repo's
own rules, this repo wins. In order:

1. **`config/guardrails.md` outranks everything.** No invented facts, no
   unverified numbers, no internal Ployo specifics, no precise customer counts.
   The humanizer's own rule 3 ("never invent facts") says the same thing. If a
   humanizing rewrite would require a specific detail you do not have, write the
   plain version instead. Never sharpen a vague claim into a concrete one.
2. **`config/persona.md` outranks the style rules below.** Its exemplar posts ARE
   the writing sample the "Voice Calibration" section asks for. Match them.
3. ⚠ **The em dash ban is ABSOLUTE and has NO sample exception.** §14 says a
   writing sample can override it. In this repo it cannot. Zero `—` and zero `–`
   in every published surface, always. This is Ahmed's standing rule.
4. **Do not flatten the things that make these posts work.** The playbook requires
   an opinion-first hook in the first ~140 chars and a strong closing line. Those
   are deliberate, and the skill's own "What NOT to flag" section already protects
   them ("one short emphatic sentence" is explicitly not a tell). Specifically:
   - §31 (staccato drama) applies to a *run* of engineered fragments, not to a
     single clipped line landing a point. Never convert the hook into a neutral
     topic sentence.
   - §33 (rhetorical openers) targets the fake-candid "Honestly?" pause-and-reveal.
     A blunt opinion stated flat is not that, and it is exactly what we want.
   - §25 (generic positive conclusions) must not be used to delete a real closing
     verdict. Kill hollow uplift, keep the stance.
   If a rewrite makes a post safer, smoother, and more forgettable, discard the
   rewrite and keep the original line.
5. **Where this file and `persona.md` §"AI tells — hard ban" overlap, they agree.**
   The overlap is intentional. This file adds the ~20 patterns persona.md does not
   list (copula avoidance, elegant variation, false ranges, negative parallelisms,
   inflated symbolism, -ing analyses, weasel attributions, aphorism formulas,
   signposting, hyphenated-pair overuse, and the rest).

## Kill switch

To disable this pass without touching the routine prompt, set
`humanizer_enabled: false` in `state/settings.json`. If that key is absent,
treat it as **true** (the pass runs). If it is present and false, skip this file
entirely and rely on `persona.md` alone.
